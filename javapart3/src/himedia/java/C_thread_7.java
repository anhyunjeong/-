...
// * 스레드의 실행 제어
// ** 스레드 상태
// [NEW] 스레드가 생성되고 아직 start()가 호출이 되지 않은 상태

// [RUNNABLE] 실행중 또는 실행 가능한 상태(실행대기)
// -> 스레드르르 생성하고 start()를 호출하면 바로 실행되는 것이 아니라 실행대기열에 저장되어 자신의 차례가 될 때까지 기다려야 한다.
// 실행대기열은 큐(queue)와 같은 구조로 먼저 실행대기열에 들어온 스레드가 먼저 실행된다.
// 주어진 실행 시간이 다 되거나 yield()를 만나면 다시 실행 대기상태가 되고 다음 차례의 스레드가 실행된다.

// [BLOCKED] : 동기화블럭에 의해서 일시정지된 상태(lock이 풀릴 때까지 기다리는 상태)

// [WAITING, TIMED-WAITING] : 스레드의 작업이 종료되지 않았지만 실행가능하지 않은 일시정지 상태
// TIMED_WAITING은 일시정지시간이 지정된 경우를 의미한다. -> 실행 중에 suspend(), sleep(), wait(), join(), I/O block에 의해 일시정지된 상태가 된다.
// I/O block은 입출력작업에서 발생하는 지연상태를 뜻한다. 사용자의 입력을 기다리는 경우를 예로 들 수 있는데, 이런 경우 일시정지 상태에 있다가 사용자가 입력을
//마치면 다시 실행대기 상태가 된다. -> 지정된 일시정지시간이 다되거나(time-out), notify(), resume(), interrupt()가 호출되면 일시 정지 상태를 벗어나
//실행대기열에 저장되어 자신의 차례를 기다리게 된다.

// [TERMINATED] : 스레드의 작업이 종료된 상태 -> 실행을 모두 마치거나 stop()이 호출되면 스레드는 소멸된다.

// * 메서드
// - sleep() : 지정된 시간동안 스레드를 일시정지 시킨다. 지정된 시간이 지나고 나면, 자동적으로 다시 실행대기 상태가 된다.
...
// - resume() : suspend()에 의해 일시정지상태에서 있는 스레드를 실행대기상태로 만든다.
...
//        }
        ...
        // 그 이유는 sleep()이 항상 현재 실행중인 스레드에 대해 작동하기 때문에 t1.sleep()을 하였어도 실제로 영향을 받는 것은 main메서드를 실행하는 main스레드이다.
        // 그래서 sleep()은 참조변수를 이용해서 호출하기보다 static으로 선언되어 있는 Thread.sleep(2000)과 같이 사용해야 한다.
    ...
    // 단지 멈추라고 요청만 하는 것일 뿐 스레드를 강제로 종료시키지는 못한다. 그리고 interrupted()는 스레드에 대해 interrupt()가 호출되었는지 알려준다.
    ...
    public static void exam2_1(){
        ...
        System.out.println("input anytihing");
    ...
    // 스레드가 sleep(), wait(), join()에 의해 '일시정지상태(WAITING)'에 있을 때, 해당 스레드에 대해 interrupted()를 호출하면, sleep(), wait(), join()에서 InterruptedException이 발생하고 스레드는
    //'실행대기 상태'로 바뀐다.
    ...
    // 즉, 실행 상태에 있는 스레드가 자신의 남은시간을 포기하고 다른 스레드에게 기회를 주도록 스케줄러에게 신호를 보내는 역할을 한다.
    // 그러나 yield() 메서드는 단순히 힌트일뿐이며, 반드시 다른 스레드가 CPU시간을 받는다는 보장이 없다.
    ...
    public static void exam4(){
        ...
        System.out.println("time period: "+(System.currentTimeMillis()-startTime)+"ms");
        // join을 사용하지 않았다면 main스레드가 바로 종료되었겠지만, join()으로 스레드 t1, t2가 모두 작업을 마칠 때까지 main스레드가 기다리도록 하였다.
    }
